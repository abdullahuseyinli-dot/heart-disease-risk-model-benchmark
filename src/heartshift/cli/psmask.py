"""Run source-only PS-MaskDRO architecture and objective ablations."""
...
from heartshift.evaluation.neural_benchmark import run_neural_inner
...
def main() -> None:
    ...
    if args.phase == "outer":
        ...
        run_name = args.run_name or f"psmask-inner-{timestamp}-{config_hash(config)[:12]}"
    run_dir = repo_root / "artifacts" / "runs" / run_name
    if args.phase == "inner":
        outputs = run_neural_inner(repo_root, config, run_dir)
    else:
        if args.confirmation != OUTER_CONFIRMATION:
            raise SystemExit(
                "Locked outer evaluation refused. Pass --confirmation RUN_LOCKED_OUTER_ONCE "
                "only after source-only selection and mechanism gates are frozen."
            )
