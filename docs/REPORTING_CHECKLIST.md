...
| Leakage controls | nested source-only selection and candidate lock | Complete in code; final lock pending |
| Model specification and hyperparameters | resolved YAML and selection tables | Source runs pending completion |
| Comparator tuning parity | source-only grids and prediction artifacts | Pending remaining runs |
| Calibration/adaptation separation | protocol and assumption-gated UDA code | Complete in code |
...
| Synthetic success/failure mechanisms | registered gate and immutable outputs | Pending run |
