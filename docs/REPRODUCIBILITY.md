...
.venv\Scripts\python.exe -m heartshift.cli.synthetic --repo-root . --config configs/synthetic/mechanism_v1.yaml --run-name synthetic-mechanism-v1
...
```powershell
.venv\Scripts\python.exe -m heartshift.cli.freeze --repo-root . --config configs/release/freeze_v1.yaml --output artifacts/locks/heartshift_candidate_v1.json
```

The lock hashes every outer configuration and Python method file plus every
source prediction/selection artifact. Outer CLIs independently verify these
hashes and the Git commit. They also enforce one frozen run name, so the same
locked test cannot silently be rerun under another directory.
...
```powershell
.venv\Scripts\python.exe -m heartshift.cli.benchmark --repo-root . --config configs/benchmark/classical_outer_v1.yaml --phase outer --inner-run-dir artifacts/runs/classical-inner-confirm-v1 --confirmation RUN_LOCKED_OUTER_ONCE
.venv\Scripts\python.exe -m heartshift.cli.benchmark --repo-root . --config configs/benchmark/modern_outer_v1.yaml --phase outer --inner-run-dir artifacts/runs/modern-inner-confirm-v1 --confirmation RUN_LOCKED_OUTER_ONCE
.venv\Scripts\python.exe -m heartshift.cli.benchmark --repo-root . --config configs/benchmark/modern_2026_outer_v1.yaml --phase outer --inner-run-dir artifacts/runs/modern-2026-inner-v1 --confirmation RUN_LOCKED_OUTER_ONCE
.venv\Scripts\python.exe -m heartshift.cli.benchmark --repo-root . --config configs/benchmark/tabpfn_v3_outer_v1.yaml --phase outer --inner-run-dir artifacts/runs/tabpfn-v3-inner-confirm-v1 --confirmation RUN_LOCKED_OUTER_ONCE
.venv\Scripts\python.exe -m heartshift.cli.psmask --repo-root . --config configs/benchmark/mirrams_outer_v1.yaml --phase outer --confirmation RUN_LOCKED_OUTER_ONCE
.venv\Scripts\python.exe -m heartshift.cli.psmask --repo-root . --config configs/benchmark/psmask_outer_v1.yaml --phase outer --confirmation RUN_LOCKED_OUTER_ONCE
.venv\Scripts\python.exe -m heartshift.cli.readmission --repo-root . --config configs/independent/readmission_outer_v1.yaml --phase outer --confirmation RUN_LOCKED_READMISSION_TEST_ONCE
```
...
```powershell
.venv\Scripts\python.exe -m heartshift.cli.report_heart --repo-root . --config configs/reporting/heart_outer_v1.yaml --output-dir artifacts/reports/heart-outer-v1
.venv\Scripts\python.exe -m heartshift.cli.report_readmission --repo-root . --config configs/reporting/readmission_outer_v1.yaml --output-dir artifacts/reports/readmission-outer-v1
```
